<script lang="ts">
  ...
  onMount(() => {
    // We automatically connect to the website's current URL (Render)
    // This replaces the "localhost" line that was breaking the connection
    const socket = io({
    ...
    socket.on('connect', () => { 
      status = "ONLINE"; 
      console.log("🛡️ Dashboard connected to Live Bridge!");
    });
    ...
    socket.on('new_whale', (data) => {
      if (data) {
        // Keeps the top 15 most recent trades
...
<main style="background: black; color: #dc2626; min-height: 100vh; padding: 40px; font-family: monospace;">
  <div style="border-bottom: 2px solid #7f1d1d; display: flex; justify-content: space-between; align-items: center; padding-bottom: 10px;">
    ...
    <span style="font-weight: bold; color: {status === 'ONLINE' ? '#22c55e' : '#dc2626'}">
      ● {status}
    </span>
  </div>
  ...
  <div style="margin-top: 30px;">
    <div style="display: grid; grid-template-columns: 1fr 1fr 1fr 2fr 1fr; border-bottom: 2px solid #450a0a; padding-bottom: 10px; font-size: 11px; opacity: 0.6; color: #D4AF37;">
      ...
      <span>ASSET</span>
      <span>PRICE</span>
      <span>VOL (CONTRACTS)</span>
      <span style="text-align: right;">SIDE</span>
    </div>
    ...
    {#each orders as o}
      <div style="display: grid; grid-template-columns: 1fr 1fr 1fr 2fr 1fr; border-bottom: 1px solid #1a1a1a; padding: 15px 0; align-items: center;">
        ...
        <span style="font-size: 1.2rem; font-weight: 800; color: white;">
          {o.vol} <small style="opacity: 0.3;">LOTS</small>
        </span>
        <span style="text-align: right; font-weight: bold; color: {o.side === 'SELL' ? '#ef4444' : '#22c55e'}">
          {o.side}
        </span>
      </div>
    {/each}
    ...
    {#if orders.length === 0}
      <div style="padding: 40px; text-align: center; color: #450a0a; font-style: italic;">
