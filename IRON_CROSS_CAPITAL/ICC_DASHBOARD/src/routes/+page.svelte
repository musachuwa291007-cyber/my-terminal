<script lang="ts">
  ...
  onMount(() => {
    ...
    socket.on('new_whale', (data) => {
      if (data) {
        // Puts the newest pair at the top
...
<main style="background: black; color: #dc2626; min-height: 100vh; padding: 40px; font-family: monospace;">
  <div style="border-bottom: 2px solid #7f1d1d; display: flex; justify-content: space-between;">
    <h1 style="font-weight: 900; margin: 0;">IRON CROSS CAPITAL</h1>
    <span style="font-weight: bold; color: {status === 'ONLINE' ? '#22c55e' : '#dc2626'}">{status}</span>
  </div>
  
  <div style="margin-top: 30px;">
    <div style="display: grid; grid-template-columns: 1fr 1fr 1fr 2fr 1fr; border-bottom: 2px solid #450a0a; padding-bottom: 10px; font-size: 11px; opacity: 0.6;">
    ...
    {#each orders as o}
      <div style="display: grid; grid-template-columns: 1fr 1fr 1fr 2fr 1fr; border-bottom: 1px solid #1a1a1a; padding: 15px 0; align-items: center;">
        ...
        <span style="font-size: 1.2rem; font-weight: 800;">{o.vol} <small style="opacity: 0.3;">LOTS</small></span>
